...
class Produse //"pentru fiecare dintre cele n produse se citesc", deci vom avea intai o clasa Produse
{
    private:
        ...
        float pret; //e float pentru ca e numar real pozitiv (deci poate fi si numar cu virgula); putem vedea si la exemple ca e un numar cu virgula
        float greutate;
    ...
    void afisare1()
    {
        std::cout<<nume<<", "; //nu punem std::endl, deoarece la output ne cere sa afisam in continuarea acestora; mare atentie si la spatiul de dupa virgula, trebuie pus neaparat
        std::cout<<categorie<<", ";
        std::cout<<pret<<" lei, "; //atentie la spatiul de dinaintea lui "lei", si de dupa virgula pusa dupa
...
int main()
{
    ...
    int n;
    std::cin>>n;
    std::cin.ignore(); //trebuie pus neaparat inaintea citirii unui sir de caractere
    ...
    if(optiune==2)
    {
        ...
        for(int i=0;i<n;i++) //pentru a le citi in ordine
            if(categorie_anume==p[i].get_categorie()) //p[i].get_categorie() apeleaza categorie din clasa Produse, folosindu-se de getter
            { //trebuie neaparat bucla, deoarece se executa 2 instructiuni simultan (deci nu se mai subintelege, ca si cand am avea doar una)
                ok++; //ok++ (echivalent cu ok=1), este folosit pentru a-i da alta valoare lui "ok", astfel incat sa afiseze doar mesajul dorit (separat de "N/A"); acesta trebuie neaparat intai incrementat cu un "for" si "if" (pentru a-i da sansa sa se modifice cat mai repede), apoi folosim valoarea data lui "ok" initial, pentru a afisa "N/A" separat 
    ...
    if(optiune==3)
    {
        int d[20]; //stocam numarul de bucati intr-un vector (trebuie sa fie minim de 20 de elemente, ca si produsele); pentru a le putea afisa dupa (deoarece nu avem niciun element numit numar_bucati in clasa) avem nevoie de un vector (de obiecte)
        ...
        { //neaparat bucla, pentru avem alt "for" dupa (care este inclus in cel actual)
    ...
    if(optiune==4)
    {
        ...
        {
            std::cout.precision(2); //pentru a afisa valoarea cu 2 zecimale exacte, avem nevoie de aceasta instructiune (un bug ciudat e ca, uneori trebuie sa dai de mai multe ori "evaluate" ca sa mearga)
            std::cout<<s<<std::endl; //afisam direct aici, deoarece in clasa nu avem o variabila de tip s (si nici nu o putem calcula acolo (daca am vrea), deoarece pentru a calcula greutatea avem nevoie de for-ul acela care il contine pe "n", iar acesta este declarat in main) 
        }
    }
    ...
    if(optiune==5)
    {
        ...
        float taxa_greutate;
        taxa_greutate=prag_greutate*cost_aditional; //putem face si prin deductie: pragul este exprimat in kg, iar costul in lei/kg, deci kg*lei/kg trebuie sa fie exprimat in lei, deci e corect
        
        float cost_produse=0; //mereu cand vrem sa adunam cu o valoare curenta (si getter), vom initializa cu 0 variabila
        
        for(int i=0;i<n;i++) //este mult mai corect cu getter sa luam costul produselor, dar puteam scrie si "float cost_produse=50" direct
            cost_produse=cost_produse+p[i].get_pret(); //adunam cu pretul, fiind in lei (logic)
        
        float cost_final;
        cost_final=cost_produse+taxa_greutate;
        std::cout.precision(2); //uneori merge si fara instructiunea asta + std::fixed, dar sunt bine de stiut
